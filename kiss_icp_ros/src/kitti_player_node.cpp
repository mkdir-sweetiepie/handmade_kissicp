...
class KITTIPlayerNode : public rclcpp::Node {
 ...
 public:
  KITTIPlayerNode() : Node("kitti_player_node"), current_frame_(0) {
    ...
    RCLCPP_INFO(get_logger(), "KITTI Player initialized");
    RCLCPP_INFO(get_logger(), "  Loaded %zu frames from %s", scan_files_.size(), data_path.c_str());
 ...
 private:
  ...
  void publishNextFrame() {
    ...
    RCLCPP_DEBUG(get_logger(), "Published frame %zu/%zu (%zu points)", current_frame_ + 1, scan_files_.size(), cloud->points.size());
